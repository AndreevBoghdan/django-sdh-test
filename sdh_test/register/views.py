...
class SdhRegistrationView(RegistrationView):
    ...
    def post(self, request):
        ...
        if form.is_valid():
            ...
            new_user.save()
            inviter_profile = Profile.objects.filter(
                invitation_code=form.cleaned_data['invitation_code']).first()
            if inviter_profile:
...
def rating(request):
    top_ten = Profile.objects.all().order_by('-points')[:10]
    return render(request, 'registration/rating.html', {'top_ten': top_ten})
